const {Crypto, AeSdk} = require('@aeternity/aepp-sdk');
...
module.exports = class PriceFeedOracle {
  ...
  register = async (queryFee = 200000000000000) => {
    ...
    if (!this.oracle) this.oracle = await this.aeternity.client.registerOracle("int", "int", {
      queryFee: queryFee,
  ...
  respond = async (queries) => {
    ...
    if (!query) return; //return early on no or non-empty response;
    const queryString = parseInt(String(Crypto.decodeBase64Check(query.query.slice(3))));
    console.log("oracle got query", queryString);

    let response = await axios.get(`https://api.coingecko.com/api/v3/simple/price?ids=aeternity&vs_currencies=usd`)
      .then(res => res.data.aeternity["usd"])

    if (response) {
      response = Math.floor(Number(response) * this.aeternity.atomsToAe(queryString) * 1000)
